        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title"><b>Country List</b></h3>


                    <a href="{{route('countries.create')}}" class="btn btn-primary float-right text-white">
                    ...
                    <table class="table table-bordered table-hover table-striped">
                        ...
                        <tbody>
                            @php
                                ...
                                $i = OwnLibrary::paginationSerial($countries);
                            @endphp
                            @forelse($countries as $countrie)
                            <tr>
                                <td>{{ $i++ }}</td>
                                @if(Auth::user()->role_id != 10)
                                    <td>{!! $countrie->createdBy->name !!}</td>
                                @endif
                                ...
                                <td class="text-center">
                                    <form method="post" action="{{route('countries.destroy',$countrie->id)}}">
                                        <a class="btn btn-sm btn-warning text-white"
                                            href="{{route('countries.edit',$countrie->id)}}" title="Edit">
                ...
                <div class="card-footer clearfix text-right">
                    {{$countries->links("backend.include.pagination")}}
                </div>
            </div>
        </div>
    </div>
